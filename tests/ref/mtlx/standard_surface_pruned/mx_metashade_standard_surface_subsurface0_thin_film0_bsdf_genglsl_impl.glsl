...
void mx_metashade_standard_surface_subsurface0_thin_film0_bsdf(ClosureData closureData, float base, vec3 base_color, float diffuse_roughness, float metalness, float specular, vec3 specular_color, float specular_roughness, float specular_IOR, float specular_anisotropy, float specular_rotation, float transmission, vec3 transmission_color, float transmission_extra_roughness, float sheen, vec3 sheen_color, float sheen_roughness, float coat, vec3 coat_color, float coat_roughness, float coat_anisotropy, float coat_rotation, float coat_IOR, vec3 coat_normal, float coat_affect_color, float coat_affect_roughness, vec3 normal, vec3 tangent, inout BSDF bsdf)
{
	// 
	...
	// Specular BSDF (dielectric reflection)
	{
		...
		mx_dielectric_bsdf(closureData, specular, specular_color, specular_IOR, main_roughness, false, 0.0, 1.5, normal, main_tangent, 0, 0, specular_bsdf);
		// 
	...
	// Artistic IOR (reflectivity/edge-color -> physical IOR/extinction)
	{
		...
		mx_conductor_bsdf(closureData, metalness, ior_n, ior_k, main_roughness, false, 0.0, 1.5, normal, main_tangent, 0, metal_bsdf);
		// 
